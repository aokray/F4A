...
def checkInBounds(domain, value, method, arg_dict = None):
    l_paren = domain[0]
    r_paren = domain[-1]
    ...
    if l_paren == '(':
        ...
        if not gt_lower:
            raise Exception('The value you enter for the '+ method +' hyperparameter must be greater than ' + str(vals[0]) + ', because the "(" symbol in the valid values means the value you enter must be strictly greater than ' + str(vals[0]) + '.')
    elif l_paren == '[':
        if types[0] == str:
            gt_lower = arg_dict[vals[0]] <= value
        else:
        ...
        if not gt_lower:
                raise Exception('The value you enter for the '+ method +' hyperparameter must be greater than or equal to ' + str(vals[0]) + ', because the "[" symbol in the valid values  means the value you enter must be greater than or equal to ' + str(vals[0]) + '.')
    else:
    ...
    if r_paren == ')':
        ...
        if not lt_upper:
            raise Exception('The value you enter for the '+ method +' hyperparameter must be less than ' + str(vals[1]) + ', because the ")" symbol in the valid values  means the value you enter must be strictly less than ' + str(vals[1]) + '.')
    elif r_paren == ']':
        if types[1] == str:
            lt_upper = arg_dict[vals[1]] >= value
        else:
